...
from util import (
    get_apikey,
    find_comfyui_processes,
    get_comfyui_install_path,
    get_comfyui_ports,
)
import subprocess
import signal
...
class Cli:
    ...
    def start_server(self):
        ...
        with open("client.log", "w") as log_file:
            p_cf = subprocess.Popen(
                [
                    ...
                    "start",
                    "model",
                    f"--upload-url=http://localhost:3000/api/cli/upload",
                ],
        ...
        try:
            if model_id:
                ...
                print(
                    f"Heartbeat service started with interval of {heartbeat_interval} seconds"
                )
        ...
        def terminate_process(name):
            try:
                ...
                result = subprocess.run(
                    ["pgrep", "-f", name], stdout=subprocess.PIPE, text=True
                )
    ...
    def parse_predict(self, predict_filename: str = "predict.py"):
        ...
        return f"Created predict_signature.json"
    ...
    def pull_project(self, project_id: str, project_name: str = None):
        ...
        try:
            ...
            if signature_data:
                with open(
                    dest_path / "predict_signature.json", "w", encoding="utf-8"
                ) as f:
            ...
            if tunnel_credential:
                with open(
                    dest_path / "tunnel_credential.json", "w", encoding="utf-8"
                ) as f:
    ...
    def pull_tunnel_credentials(self, app_id):
        response = requests.get(
            f"http://localhost:3000/api/cli/app/{app_id}",
            headers={
                "X-API-KEY": "eeac270b-8a99-4536-9557-50b02aa7cf6e",
                "Content-Type": "application/json",
            },
        )
        response.raise_for_status()  # Raise an exception for HTTP errors
        tunnel_data = response.json()["tunnelCfg"]
        # print(tunnel_data)
        with open("tunnel_config.json", "w", encoding="utf-8") as f:
            json.dump(tunnel_data["config"], f, indent=4)
        with open("tunnel_credential.json", "w", encoding="utf-8") as f:
            json.dump(tunnel_data["credential"], f, indent=4)  
